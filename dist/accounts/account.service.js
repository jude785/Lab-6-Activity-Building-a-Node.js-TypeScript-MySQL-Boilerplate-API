...
function getJwtSecret() {
    ...
    const secret = process.env.JWT_SECRET || config_1.default.secret;
    if (!secret)
        throw new Error('JWT secret is missing');
    return secret;
}
function generateJwtToken(account) {
    const token = jsonwebtoken_1.default.sign({ sub: account.id, id: account.id }, getJwtSecret(), { expiresIn: '15m' });
    return token;
}
function generateRefreshToken(account, ipAddress) {
    const refreshToken = new db_1.default.RefreshToken({
